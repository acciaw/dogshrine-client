...
function card(game) {
  ...
  const dotsMenu = makeMenu(dotsTrigger, [
    ...
    game.source === 'manual'
      ? { label: 'Clear manual path', onClick: async () => render(await window.toby.clearOverride(game.id)) }
      : null,
    {
    ...
    game.saveDirSource === 'manual'
      ? { label: 'Clear manual save folder', onClick: async () => render(await window.toby.clearSaveDirOverride(game.id)) }
      : null,
  ]);
